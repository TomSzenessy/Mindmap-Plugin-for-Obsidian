...
var { CARD_LAYOUT_VERSION, LiveSizingController } = (() => {
...
var MarkdownOrder = (() => {
  ...
  function isStandaloneBlock(value) {
    const text = String(topicNode(value)?.text || "").trim();
  ...
  function reorderPreservingSource(markdown, canvas, dependencies) {
    ...
    return withMetadata(
      ...
      desiredOrder,
      desiredOrder.map((id) => identityKey(liveById.get(id)?.text || "Untitled")),
      desiredOrder.map((id) => identityLabel(liveById.get(id)?.text || "Untitled"))
    );
  }
...
function canvasToMindMapMarkdown(canvas, options = {}) {
  ...
  const emitHeadingChildren = (children, level) => {
    ...
    const useHeadingLevel = level <= 6 && children.every((child) => {
      ...
      return isStandaloneMarkdownBlock(raw) && child.children.length === 0 || markmapHeadingSafe(raw);
    });
    if (!useHeadingLevel) {
      for (const child of children)
...
function parseMarkdownMindMapDocument(markdown) {
  ...
  const addNode = (text, parent, explicitId, source = null) => {
    ...
    while (usedIds.has(id))
      id = genId();
    usedIds.add(id);
    const node = { id, legacyId: identity.id ? id : null, text: cleanImportedTopic(identity.text), position: "right", children: [], source };
    if (parent)
...
function patchMarkdownFromCanvasPreservingSource(markdown, canvas, imported, canvasPath) {
  ...
  for (const node of liveNodes) {
    const base = headingSlug(node.text);
    const count = (slugCounts.get(base) || 0) + 1;
    slugCounts.set(base, count);
    idToSlug.set(node.id, count === 1 ? base : `${base}-${count}`);
  }
  const portableText = (node) => markdownWithPortableCardLinks(String(node.text || "").trim() || "Untitled", idToSlug);
  const normalizeText = (text) => String(text || "").replace(/\r\n?/g, "\n").trim();
  const patches = [];
  ...
  for (const id of commonIds) {
    ...
    if (normalizeText(live.text) === normalizeText(incoming.text))
      continue;
  ...
  const renderAddedTree = (node, style) => {
    ...
    const richBlock = isStandaloneMarkdownBlock(text);
    const lines = [];
    let childStyle;
    if (richBlock) {
      lines.push(text);
  ...
  const metadata = {
    ...
    topicKeys: sourceOrder.map((id) => topicIdentityKey(liveById.get(id)?.text || "Untitled")),
    topicLabels: sourceOrder.map((id) => topicIdentityLabel(liveById.get(id)?.text || "Untitled"))
  };
...
var CanvasMindMapPlugin = class extends import_obsidian5.Plugin {
  ...
  registerMediaDropHandler(canvas) {
    ...
    const activate = (event) => {
      ...
      wrapper.addClass("mindvas-drop-active");
    };
    ...
    const onDragOver = (event) => activate(event);
    const onDragLeave = (event) => {
      if (!supports(event))
        return;
      dragDepth = Math.max(0, dragDepth - 1);
      if (dragDepth === 0)
        wrapper.removeClass("mindvas-drop-active");
    };
    const onDrop = (event) => {
      if (!supports(event))
        return;
      ...
      const topic = target && typeof target.text === "string" && !getGroupIds(canvas).has(target.id)
        ? target
        : null;
  ...
  async addDroppedFiles(canvas, files, position, topic = null) {
    ...
    if (topic) {
      const embeds = createdFiles.map(({ file }) => {
        const link = this.app.fileManager.generateMarkdownLink(file, sourcePath);
        return link.startsWith("!") ? link : `!${link}`;
      });
      topic.setText([String(topic.text || "").trim(), ...embeds].filter(Boolean).join("\n\n"));
      canvas.requestSave();
      this.waitForPreview(topic, () => this.resizeNodesWhenRendered(canvas, [topic]));
      this.canvasApi.selectForNavigation(canvas, topic, this.settings.navigationZoomPadding);
    } else {
      const nodes = [];
      let cursorY = Number(position?.y) || 0;
      const x = Number(position?.x) || 0;
      for (const { file, mimeType } of createdFiles) {
        const size = MediaDrop.mediaNodeSize(file.name, mimeType, this.settings);
        let id = genId();
        ...
        cursorY += size.height + this.settings.verticalGap;
      }
  ...
  addDroppedUrl(canvas, url, position, topic = null) {
    if (topic) {
      const label = MediaDrop.linkLabel(url).replace(/[\[\]]/g, "");
      const link = `[${label}](<${url}>)`;
      topic.setText([String(topic.text || "").trim(), link].filter(Boolean).join("\n\n"));
      canvas.requestSave();
      this.waitForPreview(topic, () => this.resizeNodesWhenRendered(canvas, [topic]));
      this.canvasApi.selectForNavigation(canvas, topic, this.settings.navigationZoomPadding);
      return;
    ...
    canvas.importData({
      nodes: [{
        ...
        width: Math.min(this.settings.maxNodeWidth, Math.max(this.settings.minNodeWidth, 480)),
        height: Math.min(this.settings.maxNodeHeight, Math.max(this.settings.defaultNodeHeight, 280))
      }],
      edges: []
    });
    this.canvasApi.invalidateEdgeIndex();
  ...
  injectToggleButton(canvas) {
    ...
    const mediaBtn = ownerDocument.createElement("div");
    mediaBtn.addClass("mindvas-media-btn", "clickable-icon");
    mediaBtn.setAttribute("aria-label", "Add images, PDFs, or media");
    (0, import_obsidian5.setIcon)(mediaBtn, "image-plus");
    this.registerDomEvent(mediaBtn, "click", (event) => {
      event.stopPropagation();
      this.openMediaFilePicker(canvas);
    });
    controls.prepend(mediaBtn);
    this.mediaBtnEl = mediaBtn;
    this.updateToggleButton(canvas);
  }
