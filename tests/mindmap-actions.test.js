...
const { separateBranch, colorBranch, toggleSubtreeCollapse } = require("../lib/mindmap-actions.js");
...
test("colors node and descendants recursively", () => {
  const rootNode = { id: "root", setColor: (c) => { rootNode.color = c; } };
  const childNode = { id: "child", setColor: (c) => { childNode.color = c; } };
...
test("toggles subtree collapse state", () => {
  const rootNode = {
    ...
    setData: (data) => { rootNode.data = data; }
  };
  const childNode = {
    id: "child",
    nodeEl: {
      addClass: (cls) => { childNode.class = cls; },
      removeClass: (cls) => { childNode.class = ""; }
    }
  };
  ...
  assert.equal(childNode.class, "mindvas-collapsed-hidden");
});
